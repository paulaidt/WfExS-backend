...
class HintedWorkflowComponent(NamedTuple):
    """
    relPath: relative path of the component. The reference is the
        workflow directory, so these tuples can be resolved
        only along with its parent LocalWorkflow instance
    role: The role of the file, which is a hint for the own engine
    materializedPath: In case the relPath is an URI, and the engine
        has had to locally materialize it in order to process it,
        its local path
    """
    relPath: "Union[RelPath, URIType]"
    role: "str"
    materializedPath: "Optional[AnyPath]" = None
